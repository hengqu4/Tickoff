    ...
    <el-header style="position: relative;margin-top:20px">
      <el-tabs v-model="activeName" type="card" @tab-click="handleClick">
        <el-tab-pane label="待办列表" name="todo">
          <meeting-list :meetingList="todoMeetingList"></meeting-list>
        </el-tab-pane>
...
import { getMeetings, getTodoMeeting, getHistoryMeeting } from "@/api/project";
import {
  searchMeeting,
  getMyMeeting,
  createMeeting
...
export default {
  ...
  methods: {
    ...
    getTodoMeeting(projectId) {
      getTodoMeeting(projectId).then(res => {
        console.log(res.data)
        this.todoMeetingList = res.data;
      });
    },
    ...
    async getMeeting(projectId) {
      getMeetings(projectId).then(res => {
